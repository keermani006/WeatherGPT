...
async def chat(request: Request, body: ChatRequest) -> ChatResponse:
    ...
    if clean_msg in _GREETINGS:
        logger.info("Greeting received; returning friendly welcome answer")
        return ChatResponse(
            answer=(
                "Hello! I'm WeatherGPT, your conversational weather AI. "
                "You can ask me about current conditions, forecasts, precipitation, or temperature "
                "for any city worldwide (for example, 'Weather in London' or 'Will it rain today?')."
            ),
            location="Global",
            weather_data=None,
        )
    ...
    has_location_intent = bool(body.location or extract_location_from_message(body.message))
