...
def test_decode_missing_secret():
    exp = int(time.time()) + 3600
    token = _make_token({"sub": "user-456", "exp": exp})
    with patch.object(get_settings(), "supabase_jwt_secret", ""), patch.object(get_settings(), "supabase_secret_key", ""):
...
def test_auth_demo_token_endpoint():
    from app.main import app
    c = TestClient(app)
    with patch.object(get_settings(), "supabase_jwt_secret", TEST_SECRET):
        resp = c.post("/api/v1/auth/demo-token")
        assert resp.status_code == 200
        data = resp.json()
        assert "access_token" in data
        assert data["token_type"] == "bearer"
        assert data["email"] == "demo@weathergpt.local"

        # Verify /me with this token
        me_resp = c.get("/api/v1/auth/me", headers={"Authorization": f"Bearer {data['access_token']}"})
        assert me_resp.status_code == 200
        assert me_resp.json()["user_id"] == data["user_id"]
...
def test_auth_register_and_login_flow():
    ...
    with patch.object(get_settings(), "supabase_jwt_secret", TEST_SECRET):
        # 1. Register
        reg_resp = c.post("/api/v1/auth/register", json={
            "email": unique_email,
            "password": pwd,
            "name": "Alex Test",
        })
        ...
        login_resp = c.post("/api/v1/auth/login", json={
            "email": unique_email,
            "password": pwd,
        })
        ...
        bad_login = c.post("/api/v1/auth/login", json={
            "email": unique_email,
            "password": "WrongPassword999",
        })
        assert bad_login.status_code == 401
        assert bad_login.json()["detail"]["error"]["code"] == "INVALID_CREDENTIALS"

        # 5. Duplicate registration
        dup_reg = c.post("/api/v1/auth/register", json={
            "email": unique_email,
            "password": pwd,
        })
        assert dup_reg.status_code == 400
        assert dup_reg.json()["detail"]["error"]["code"] == "USER_ALREADY_EXISTS"
